	...
	while (1) {
		...
		if (strcmp(userInput, "i") == 0 || strcmp(userInput, "I") == 0) {
			do {
				interactiveMode();
				while (1) {
					printf("Do you want to encode/decode more text? y/n=> ");	// Asks if the user wants another cipher of texts
					gets(continueKey);
					// INPUT VALIDATIONS
			...
			} while (strcmp(continueKey, "y") == 0);	// Compares string to determine the continuity of the while loop 
